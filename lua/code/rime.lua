...
function M.create_inoremap_esc(key)
	vim.keymap.set("i", key, "<cmd>stopinsert<cr>", { desc = "Stop insert", noremap = true, buffer = true })
end
...
function M.create_inoremap_undo(key)
	...
	vim.keymap.set("i", key, function()
		...
		if not entry.input or not entry.cmp then
			return fallback()
		end
		...
		if not content_before:match(entry.cmp .. "$") then
			return fallback()
		end
...
function M.toggle_rime(client)
	...
	if not client or client.name ~= "rimels" then
		return
	end
